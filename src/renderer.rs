...
pub fn render_tex(paper: &PaperData, output: &Path, template: &str) -> Result<PathBuf> {
    ...
    // Preamble
    tex.push_str(&templates::get_preamble(template, &paper.title, &paper.author_line));
    ...
    tex.push_str(&format!(
        "{} \\texttt{{{}}}\n\n",
        if paper.authors.len() > 1 { "Correspondence: " } else { "" },
...
fn inject_metadata(pdf_path: &Path, tex_path: &Path) -> Result<()> {
    ...
    let info = lopdf::Dictionary::from_iter([
        ...
        (b"Subject".to_vec(), Object::string_literal("Generated by PaperCoat - Computational Literary Analysis")),
    ]);
...
/// Render a "stats-only" text summary (no academic transformation)
pub fn render_stats(text: &str) -> String {
    let stats = crate::transformer::compute_readability_stats(text);
